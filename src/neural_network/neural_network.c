                                             ...
                                             int arr[size_of_arr])
{
    ...
    for (int i = 2; i < size_of_arr; i++)
    {
        ...
        link_layers(&neural_network->layers[i - 2],
                    &neural_network->layers[i - 1]);
    }
                     ...
                     char expected_output)
{
    ...
    return -logf(
        (*(neural_network->outputs))[expected_output - 'A']); // + 1e-15f);
    /*float res = 0.0;
...
int backprop_update_5(struct neural_network *neural_network,
                      char expected_output, float learning_rate)
{
    if (!neural_network || neural_network->number_of_layers <= 0)
        return -1;
    ...
    for (int li = 0; li < L; ++li)
    {
        ...
        float *input_arr = (li == 0) ? neural_network->inputs
                                     : neural_network->layers[li - 1]->outputs;
        if (!input_arr)
        {
            for (int t = 0; t < L; ++t)
                free(deltas[t]);
                      ...
                      char expected_output, float learning_rate)
{
    ...
    // printf("1\n");
    float *sigma_L_k =
        malloc(sizeof(float) * neural_network->output_size); // dE / dz^L_k
    for (int i = 0; i < neural_network->output_size; i++)
    {
        sigma_L_k[i] =
            ((*(neural_network->outputs))[i] - (i == (expected_output - 'A')) +
             1e-15f); // cross-entropy loss with softmax
        // printf("sigma_L_k[%d]=%f\t neural_network->outputs[%d]=%f\t
        // expected_output=%c actual_output=%c %c\n", i, sigma_L_k[i], i,
        // (*neural_network->outputs)[i], expected_output,
        // get_neural_network_output(neural_network), i + 'A');
        if (isnanf(sigma_L_k[i]) || isinff(sigma_L_k[i]) ||
            sigma_L_k[i] > 1.0e5 || sigma_L_k[i] < -1.0e5)
        {
            errx(EXIT_FAILURE,
                 "sigma_L_k[%d] = %f\t neural_network->outputs[%d]=%f\t "
                 "expected_output=%c\n",
                 i, sigma_L_k[i], i, (*neural_network->outputs)[i],
                 expected_output);
        }
    }
    // printf("2\n");
    float *aL1j = neural_network->layers[neural_network->number_of_layers - 2]
                      ->outputs; // a^(L-1)_j

    float **dE_WLkj =
        malloc(sizeof(float *) * neural_network->output_size); // dE / dW^L_kj
        ...
        dE_WLkj[k] =
            malloc(sizeof(float) *
                   neural_network->layers[neural_network->number_of_layers - 1]
                       ->prev_layer_size);
        for (int j = 0;
             j < neural_network->layers[neural_network->number_of_layers - 1]
                     ->prev_layer_size;
             j++)
        {
            dE_WLkj[k][j] = sigma_L_k[k] * aL1j[j];
            // if(dE_WLkj[k][j] > 0.001 || dE_WLkj[k][j] < -0.001)
            //    printf("dE_WLkj[%d][%d]=%f\t sigma_L_k[k]=%f\t
            //    aL1j[j]=%f\t\tsigma_L_k[i]=%f\n", k, j, dE_WLkj[k][j],
            //    sigma_L_k[k], aL1j[j], sigma_L_k[k]);
            if (isnanf(dE_WLkj[k][j]) || isinff(dE_WLkj[k][j]) ||
                dE_WLkj[k][j] > 1.0e5 || dE_WLkj[k][j] < -1.0e5)
            {
                errx(EXIT_FAILURE,
                     "dE_WLkj[%d][%d] = %f\t sigma_L_k[k]=%f\t aL1j[j]=%f\n", k,
                     j, dE_WLkj[k][j], sigma_L_k[k], aL1j[j]);
            }
        }
    }
    // printf("3\n");
    ...
    for (int k = 0; k < neural_network->output_size; k++)
    {
        ...
        {
            neural_network->layers[neural_network->number_of_layers - 1]
                ->weights[k][j] -= learning_rate * dE_WLkj[k][j];
            // if( -0.001 < (- learning_rate * dE_WLkj[k][j]) || (-
            // learning_rate * dE_WLkj[k][j]) > 0.001) {
                        ...
                        ->weights[k][j] < -1.0e5)
            {
                errx(
                    EXIT_FAILURE,
                    "neural_network->layers[%d]->weights[%d][%d] = %f\t "
                    "dE_WLkj[k][j]=%f\n",
                    neural_network->number_of_layers - 1, k, j,
                    neural_network->layers[neural_network->number_of_layers - 1]
                        ->weights[k][j],
                    dE_WLkj[k][j]);
            }
        ...
        neural_network->layers[neural_network->number_of_layers - 1]
            ->biases[k] -= learning_rate * dE_bLk[k];
        if (isnanf(neural_network->layers[neural_network->number_of_layers - 1]
                       ->biases[k]) ||
            isinff(neural_network->layers[neural_network->number_of_layers - 1]
                       ->biases[k]) ||
            neural_network->layers[neural_network->number_of_layers - 1]
                    ->biases[k] > 1.0e5 ||
            neural_network->layers[neural_network->number_of_layers - 1]
        ...
        {
            errx(EXIT_FAILURE,
                 "neural_network->layers[%d]->biases[%d] = %f\t dE_bLk[k]=%f\n",
                 neural_network->number_of_layers - 1, k,
                 neural_network->layers[neural_network->number_of_layers - 1]
                     ->biases[k],
                 dE_bLk[k]);
        }
    ...
    for (int l = neural_network->number_of_layers - 1; l > 0; l--)
    {
        ...
        float *sigma_l1_j =
            malloc(sizeof(float) *
                   neural_network->layers[l - 1]->layer_size); // sigma^(l-1)_j
        for (int j = 0; j < neural_network->layers[l - 1]->layer_size; j++)
        {
            ...
            {
                sigma_l1_j[j] +=
                    sigma_L_k[k] * neural_network->layers[l]->weights[k][j];
            }
            sigma_l1_j[j] *= deriv_ReLU_activation_function(
                neural_network->layers[l - 1]->z[j]);
        }
        // printf("\t2 %d\n", l);
        float **dE_Wlkj =
            malloc(sizeof(float *) *
                   neural_network->layers[l]->layer_size); // dE / dW^(l-1)_jk
        for (int k = 0; k < neural_network->layers[l]->layer_size; k++)
        {
            dE_Wlkj[k] = malloc(sizeof(float) *
                                neural_network->layers[l]->prev_layer_size);
            for (int j = 0; j < neural_network->layers[l - 1]->layer_size; j++)
            {
                dE_Wlkj[k][j] =
                    sigma_L_k[k] * neural_network->layers[l - 1]->outputs[j];
                // printf("dE_Wlkj[%d][%d]=%f\t sigma_L_k[k]=%f\t
                // neural_network->layers[l-1]->outputs[j]=%f\n", k, j,
                // dE_Wlkj[k][j], sigma_L_k[k],
                // neural_network->layers[l-1]->outputs[j]);
            }
        }
        // printf("\t3 %d\n", l);
        ...
        { // Apply the gradients
            ...
            {
                // printf("neural_network->layers[l]->weights[k][j]=%f\t->\t
                // %f\tdelta=%f\n", neural_network->layers[l]->weights[k][j],
                // neural_network->layers[l]->weights[k][j] - learning_rate *
                // dE_Wlkj[k][j], - learning_rate * dE_Wlkj[k][j]);
                neural_network->layers[l]->weights[k][j] -=
                    learning_rate * dE_Wlkj[k][j];
                if (isnanf(neural_network->layers[l]->weights[k][j]) ||
                    isinff(neural_network->layers[l]->weights[k][j]) ||
                    neural_network->layers[l]->weights[k][j] > 1.0e5 ||
                    neural_network->layers[l]->weights[k][j] < -1.0e5)
                {
                    errx(EXIT_FAILURE,
                         "neural_network->layers[%d]->weights[%d][%d] = %f\t "
                         "dE_Wlkj[k][j]=%f\n",
                         l, k, j, neural_network->layers[l]->weights[k][j],
                         dE_Wlkj[k][j]);
                }
            }
            neural_network->layers[l]->biases[k] -= learning_rate * dE_blk[k];
            if (isnanf(neural_network->layers[l]->biases[k]) ||
                isinff(neural_network->layers[l]->biases[k]) ||
                neural_network->layers[l]->biases[k] > 1.0e5 ||
                neural_network->layers[l]->biases[k] < -1.0e5)
            {
                errx(EXIT_FAILURE,
                     "neural_network->layers[%d]->biases[%d] = %f\t "
                     "dE_blk[k]=%f\n",
                           ...
                           int success)
{
    ...
    fprintf(file,
            "Learning Rate: %f\t| Batch Size: %d\t| Number of sessions : %d\t| "
            "Number of Success: %d\n##############################\n",
