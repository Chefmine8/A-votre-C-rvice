...
/*
il faut une fonction qui prend en paramétre le lien où ce trouve les images des
lettres, tu parcours toutes les images et tu les transmets au réseaux puis tu
prend le proba la pluis élevé du reseau et tu le met dans un fichier txt qui
...
{
    const char *partial_path = "./output/";
    char *id = identifiant;
    DIR *dir;
    struct dirent *entry;
...
void create_grid(char *id, struct neural_network *neural_net)
{
    ...
    for (int i = 1; i <= row; i++) // row
    {
        ...
        {
            sprintf(letter, "%scell_%d_%d.png", full_path, j,
                    i); // reversed for the Q
...
{
    const char *partial_path = "./output/word_list";
    DIR *dir;
    struct dirent *entry;
