...
int single_train_cession(struct neural_network *neural_network,
                         float learning_rate, int batch_size)
{
    ...
    for (int b = 0; b < batch_size && returned_value == 0; b++)
    {
        ...
        if (img != NULL)
        {
            ...
            returned_value = backprop_update_4(neural_network, expected_output,
                                               learning_rate);
        }
    }
...
int all_dataset_train_session(struct neural_network *neural_network,
                              float learning_rate)
{
    int returned_value = 0;
    for (int i = 0; i < 26 && returned_value == 0; i++)
    {
        for (int j = 0; j < 570 && returned_value == 0; j++)
        {
            ...
            {
                get_nn_input(img, neural_network->inputs);
                returned_value = backprop_update_4(
                    neural_network, expected_output, learning_rate);
            }
        }
    }
    return returned_value;
...
int test_neural_network(struct neural_network *neural_network)
{
    ...
    // ((float)total_success) / (28*26) * 100.0);
...
int main()
{
    ...
    */
    ...
    for (int i = 0; i < nb_sessions && returned_value == 0; i++)
    {
        ...
        returned_value = all_dataset_train_session(
            neural_network, learning_rate); //, batch_size);
        // printf("%f%\n", (float)i /(float) nb_sessions);
        // returned_value = single_train_cession(neural_network, learning_rate,
        // batch_size); learning_rate *= 0.99;
        success = test_neural_network(neural_network);
    ...
    success = test_neural_network(neural_network);
    export_neural_network(neural_network, learning_rate, batch_size,
                          nb_sessions, success);
