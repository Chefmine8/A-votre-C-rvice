...
Image *manual_image_scaling(const Image *src, const float scale_x,
                            const float scale_y)
{
    if (!src)
        return NULL;
...
Image *resize_image(const Image *src, const int width, const int height,
...
void get_nn_input(const Image *img, float *input)
{
    ...
    for (int j = 0; j < resized->height; j++)
    {
        for (int i = 0; i < resized->width; i++)
        {
            ...
            // Convert to grayscale using standard luminance formula and
            // normalize to [0, 1]
            long double gray =
