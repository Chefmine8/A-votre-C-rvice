...
double get_auto_rotation_angle(const Image *img)
{
    ...
    // //export the circle image for debug use randon id in filename to avoid
    // overwrite SDL_Surface *circle_surf = image_to_sdl_surface(circle_img);
    // char circle_filename[256];
    // snprintf(circle_filename, sizeof(circle_filename),
    // "../../resources/pre_process/output/circle_image_%ld.bmp", random());
    ...
    // "../../resources/pre_process/output/horizontal_lines_%ld.bmp", random());
